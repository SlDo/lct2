...
import { CameraListItem } from '@pages/Home/components/Menu/components/CameraList/components';
import { type MenuProps } from '@pages/Home/components/Menu/types';
    ...
    <>
      {!isMobile ? <Styled.MenuWrapper>
        ...
        {cameraList?.length === 0 || !cameraList ? <Flex fullWidth justifyContent={FlexJustifyContent.CENTER}><Spinner /></Flex> : (
            <Styled.List>
              {cameraList?.map(({ id, icon, address }) => <Styled.ListLink key={id} to={ROUTES.video.path.replace(':id', String(id))}><CameraListItem
